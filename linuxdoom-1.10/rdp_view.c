...
#if N64_BENCH_MARKS
    {
        // Block-integrity hash for a host cross-check: FNV-1a over the whole
        // scattered CI8 block. A PU_CACHE eviction mid-scatter (the bug the
        // pin above closes) shows up as a mismatch vs the WAD-decoded truth.
        uint32_t bh = 2166136261u;
        int bn = m->rowb * m->mh;
        for (i = 0; i < bn; i++) { bh ^= m->block[i]; bh *= 16777619u; }
        debugf("SPRBLK lump=%d w=%d h=%d rowb=%d key=%d fnv=%08x\n",
               sprlump, m->mw, m->mh, m->rowb, m->key, (unsigned)bh);
    }
#endif
